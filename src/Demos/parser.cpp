...
public:
  ...
  void defineShapes(Shapes *S) {
	...
	for (int j=0; j<(int)contours_poly[i].size();j++) {
  ...
  void draw() {
    // simplified contours
    vector<vector<Point> > contours_poly( (int)contours.size() ); 
    for( int i = 0; i< (int)contours.size(); i++ ) {
      approxPolyDP( Mat(contours[i]), contours_poly[i], 1.1, true ); 
    }
    /// Draw contours
	...
	for (int j=0; j<(int)contours_poly[i].size();j++) {
	  printf("(%i,%i)",contours[i][j].x,contours[i][j].y);
	  if (j!=(int)contours_poly[i].size()-1) { printf(","); }
	}
	printf("\n");
      }
    }
...
public:
  ...
  void defineRegionsInImage(Mat *image) {
	...
	if (found_color) { 
	  ...
	  if ((region.size()>=min_region_pixels) {
	    ...
	    set_color[1]=255;
	    set_color[2]=0;
	    for (int i=0; i<(int)region.size(); i++) { image->at<Vec3b>(Point(region[i].x,region[i].y))=set_color; }
	    num_regions--;
	  }
	...
	for (int i=0; i<cols; i++) {
	  for (int j=0; j<rows; j++) {
	    ...
	    int size=(int)regions[region_id].size();
		...
		for (int m=top; m<=bottom; m++) {
		  if (n!=i || m!=j) {
		    int size=(int)regions[done[n*rows+m]].size();
		...
		//int remove_index=-1;
		for (int p=0; p<=(int)regions[region_id].size(); p++) {
		...
		//continue; //p=(int)regions[region_id].size()+1; // get out of loop so can only remove 1 point from list
  ...
  void draw() {
    ...
    for (int r=0; r<(int)regions.size(); r++) {
      int num_pixels=(int)regions[r].size();
  ...
  void defineShapes(Shapes *S) {
	...
	for (int p=0; p<(int)regions[r].size(); p++) {
