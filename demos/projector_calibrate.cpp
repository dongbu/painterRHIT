...
// webcam
Point2f inputQuadW[4]; // Input Quadilateral or Image plane coordinates
Point2f outputQuadW[4]; // Output Quadilateral or World plane coordinates
...
void webcamWindowMouseCallBackFunc(int event, int x, int y, int flags, void* userdata)
{
  if  ( event == EVENT_LBUTTONDOWN ) {
    ...
    inputQuadW[g_webcam_corner] = Point2f( x,y );
...
void projectorWindowMouseCallBackFunc(int event, int x, int y, int flags, void* userdata)
{
  if  ( event == EVENT_LBUTTONDOWN ) {
    ...
    inputQuadP[g_projector_corner] = Point2f( x,y );
  }
}
...
int main(int, char**)
{
  ...
  inputQuadW[0] = Point2f( 0,0 );
  inputQuadW[1] = Point2f( webcam.cols-0,0);
  inputQuadW[2] = Point2f( webcam.cols-0,webcam.rows-0);
  inputQuadW[3] = Point2f( 0,webcam.rows-0  ); 
  ...
  outputQuadW[0] = Point2f( 0,0 );
  outputQuadW[1] = Point2f( g_canvas_pixels_width,0);
  outputQuadW[2] = Point2f( g_canvas_pixels_width,g_canvas_pixels_height);
  outputQuadW[3] = Point2f( 0,g_canvas_pixels_height );
  ...
  while (!webcam_done)
	...
	flip(webcam,webcam,-1); // horizontal and vertically
      ...
      webcam_lambda = getPerspectiveTransform( inputQuadW, outputQuadW );
  ...
  // The 4 points that select quadilateral on the input , from top-left in clockwise order
  // These four pts are the sides of the rect box used as input
  inputQuadP[0] = Point2f( 0,0 );
  inputQuadP[1] = Point2f( webcam_corrected.cols-0,0);
  inputQuadP[2] = Point2f( webcam_corrected.cols-0,webcam_corrected.rows-0);
  inputQuadP[3] = Point2f( 0,webcam_corrected.rows-0  ); 
  //printf("Input is %i by %i\n",input.cols,input.rows);
  printf("Click on upper left corner of canvas (press 1,2,3,4 to pick corner)\n");

  // The 4 points where the mapping is to be done , from top-left in clockwise order
  outputQuadP[0] = Point2f( 0,0 );
  outputQuadP[1] = Point2f( projector_corrected.cols-0,0);
  outputQuadP[2] = Point2f( projector_corrected.cols-0,projector_corrected.rows-0);
  outputQuadP[3] = Point2f( 0,projector_corrected.rows-0  ); 

  //  outputQuadP[0] = Point2f( 100,100 );
  //outputQuadP[1] = Point2f( 300,100);
  //outputQuadP[2] = Point2f( 300,300);
  //outputQuadP[3] = Point2f( 100,300);
  ...
  while (!projector_done)
	...
	flip(webcam,webcam,-1); // horizontal and vertically
      ...
      // Get the Perspective Transform Matrix i.e. lambda
      projector_lambda = getPerspectiveTransform( inputQuadP, outputQuadP );

      // Apply the Perspective Transform for the webcam
      warpPerspective(projector_image, projector_corrected,projector_lambda,projector_corrected.size() );
      imshow("projector corrected", projector_corrected);
