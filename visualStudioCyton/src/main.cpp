   ...
   )
{
	...
	vector<vector<Point3i>> outputPoints;
	for (vector<Point> i : contours) {
		outputPoints.push_back(vector<Point3i>());
		cout << "new path" << endl;
		for (Point j : i) {
			Point3i k = Point3i(j.x, j.y, 0);
			k = k + offset;
			k = k*scale;
